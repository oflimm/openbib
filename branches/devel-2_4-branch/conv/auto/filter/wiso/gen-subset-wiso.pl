...
print "### $pool: Bestimme Titel-ID's anhand des Titelkennzeichens 'wiso' in 4712\n";

my $request=$dbh->prepare("select distinct tit.id as titid from tit where category=4712 and content='wiso'") or $logger->error($DBI::errstr);
