...
sub mail_confirmation {
    ...
    unless (Email::Valid->address($loginname)){
        OpenBib::Common::Util::print_warning($msg->maketext("Sie haben keine gütige Mailadresse eingegeben. Gehen Sie bitte [_1]zurück[_2] und korrigieren Sie Ihre Eingabe","<a href=\"http://$config->{servername}$config->{base_loc}/$view/$config->{selfreg_loc}?action=show\">","</a>"),$r,$msg);
        return Apache2::Const::OK;
    }
    
    if ($user->user_exists($loginname)) {
        OpenBib::Common::Util::print_warning($msg->maketext("Ein Benutzer mit dem Namen [_1] existiert bereits. Haben Sie vielleicht Ihr Passwort vergessen? Dann gehen Sie bitte [_2]zurück[_3] und lassen es sich zumailen.","$loginname","<a href=\"http://$config->{servername}$config->{base_loc}/$view/$config->{selfreg_loc}?action=show\">","</a>"),$r,$msg);
        return Apache2::Const::OK;
    }
    ...
    if ($config->{recaptcha_private_key}){
        ...
        unless ( $recaptcha_result->{is_valid} ) {
            OpenBib::Common::Util::print_warning($msg->maketext("Sie haben ein falsches Captcha eingegeben! Gehen Sie bitte [_1]zurück[_2] und versuchen Sie es erneut.","<a href=\"http://$config->{servername}$config->{base_loc}/$view/$config->{selfreg_loc}?action=show\">","</a>"),$r,$msg);
            return Apache2::Const::OK;
        }
    }
