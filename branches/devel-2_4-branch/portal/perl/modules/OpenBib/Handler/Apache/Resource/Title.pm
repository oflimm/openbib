...
sub handler {
    ...
    my $basepath = $config->{base_loc}."/$view/".$config->{handler}{resource_title_loc}{name};
    $path=~s/$basepath//;
    ...
    if ($path=~m/^\/([^\/]+?)\/([^\/]+?)\/([^\/]*)/){
        ...
        $logger->debug("No matching resource scheme");
        return Apache2::Const::OK;        
    }


    $logger->debug("Type: title - Database: $database - Key: $id - Representation: $representation");
