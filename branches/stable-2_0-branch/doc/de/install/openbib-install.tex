...
\begin{enumerate}
\item Installation eines Apache-Webservers mit mod\_perl-Unterstuetzung in
   der Version 1.3.x.
   ...
   \item Verzeichnisname des conf.d-Verzeichnisses, in dem individuelle
...
\item Installation der fuer OpenBib benoetigten Perl-Module fuer Perl ab
...
\item Gehen Sie nun in das ausgecheckte CVS-Verzeichniss und passen
   ...
   \begin{description}
   \item[\$documentroot] Wo ist der Basis-Pfad fuer die Dokumente Ihres
                   Apache-Webservers

   \item[\$confd]        Wo liegt das conf.d-Verzeichnis Ihres
                   Apache-Webservers
   \end{description}

   Beide Pfade sind ohne abschliessenden '/' einzugeben.

   Danach erzeugen sie bitte mit 'touch' die Datei '.changed\_config'
...
\item Rufen Sie nun das Programm quickinstall.pl als ein Nutzer auf, der
...
\item Bitte passen Sie nun die Datei
   ...
   \begin{verbatim}
   /usr/local/lib/site\_perl/OpenBib/Config.pm
   \end{verbatim}
...
\item Danach fuehren Sie bitte folgende Programme zur Erzeugung der
