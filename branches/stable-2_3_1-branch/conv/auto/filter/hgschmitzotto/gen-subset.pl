...
print "### $pool: Bestimme Titel-ID's anhand des Signaturanfangs HGSO\n";

my $request=$dbh->prepare("select distinct conn.sourceid as titid from conn,mex where mex.category=14 and mex.content rlike '^HGSO' and conn.targetid=mex.id and conn.sourcetype=1 and conn.targettype=6") or $logger->error($DBI::errstr);
