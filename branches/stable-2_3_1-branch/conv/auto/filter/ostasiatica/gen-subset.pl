...
my $request=$dbh->prepare("select distinct conn.sourceid as titid from conn,notation where notation.category=1 and notation.content = 'Slg. Ostasiatica' and conn.targetid=notation.id and conn.sourcetype=1 and conn.targettype=5") or $logger->error($DBI::errstr);
