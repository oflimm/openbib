...
print "### $pool: Bestimme Titel-ID's anhand des Signaturanfaenge VOR, VORG\n";

my $request=$dbh->prepare("select distinct conn.sourceid as titid from conn,mex where mex.category=14 and (mex.content rlike '^VOR[0-9]' or mex.content rlike '^VORG[0-9]') and conn.targetid=mex.id and conn.sourcetype=1 and conn.targettype=6") or $logger->error($DBI::errstr);
