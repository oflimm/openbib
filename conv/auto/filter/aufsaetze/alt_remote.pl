...
my $exportfile = $dbinfo->remotepath."/".$dbinfo->titlefile;

print "### $pool: Datenabzug von $exportfile\n";

system("cd $pooldir/$pool ; rm meta.* *.mrc");
system("cp $exportfile $pooldir/$pool/$filename");
...
system("cd $pooldir/$pool; $marcjson2marcmetaexe --database=$pool -reduce-mem --inputfile=${filename}.processed --configfile=/opt/openbib/conf/koha.yml; gzip meta.*");
