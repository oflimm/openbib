...
my $mbox2metaexe  = "$konvdir/mbox2meta.pl";

my $pool          = $ARGV[0];
my $dbinfo = $config->get_databaseinfo->search_rs({ dbname => $pool })->single;
my $filename = $dbinfo->titlefile;

print "### $pool: Umwandlung\n";
system("cd $pooldir/$pool ; rm meta.*");
system("cd $pooldir/$pool; $mbox2metaexe --inputfile=$filename --configfile=/opt/openbib/conf/bcw1.yml ; gzip meta.*");
