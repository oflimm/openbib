...
while (<HOLDING>){
    ...
    if (defined $holding_ref->{fields}{'0014'} && $holding_ref->{fields}{'0014'}[0]{content} =~/^M/){
...
while (<HOLDING>){
    ...
    if (defined $titleids_with_mark_ref->{$titleid} && defined $holding_ref->{fields}{'0014'} && $holding_ref->{fields}{'0014'}[0]{content} !~/^M/){
