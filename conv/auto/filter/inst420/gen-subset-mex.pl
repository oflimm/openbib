...
my $pool       = $ARGV[0];
my $masterpool = "inst420master";

$mysqlexe="/usr/bin/mysql -u $config->{'dbuser'} --password=$config->{'dbpasswd'} -f";

my $dbh=DBI->connect("DBI:$config->{dbimodule}:dbname=$masterpool;host=$config->{dbhost};port=$config->{dbport}", $config->{dbuser}, $config->{dbpasswd}) or $logger->error_die($DBI::errstr);

# IDN's der Exemplardaten und daran haengender Titel bestimmen

my $signaturanfang = "^42[014] \\$.*";
...
open(AUT,"gzip -dc $pooldir/$masterpool/unload.PER.gz|");
...
open(KOR,"gzip -dc $pooldir/$masterpool/unload.KOE.gz|");
...
open(NOTA,"gzip -dc $pooldir/$masterpool/unload.SYS.gz|");
...
# Schlagworte
open(SWT,"gzip -dc $pooldir/$masterpool/unload.SWD.gz|");
...
open(TIT,"gzip -dc $pooldir/$masterpool/unload.TIT.gz|");
...
open(MEX,"gzip -dc $pooldir/$masterpool/unload.MEX.gz|");
