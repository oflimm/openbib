...
system("cd $datadir/$pool ; cat meta.title | $rootdir/filter/$pool/process_collection.pl | $rootdir/filter/$pool/restrict_jesuiten.pl > meta.title.tmp ; mv -f meta.title.tmp meta.title");
