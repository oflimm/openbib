...
print "### $pool: Entfernung des Standorts USB-Magazin\n";

system("cd $pooldir/$pool ; zcat meta.holding.gz| $rootdir/filter/$pool/remove-location.pl | gzip > meta.holding.gz.tmp ; mv -f meta.holding.gz.tmp meta.holding.gz");
