...
system("cd $datadir/$pool ; cat meta.title | $rootdir/filter/$pool/restrict_kloster.pl | $rootdir/filter/$pool/add-additionalnames.pl > meta.title.tmp ; mv -f meta.title.tmp meta.title");
