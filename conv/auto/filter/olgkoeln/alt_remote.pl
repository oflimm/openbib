...
print "### $pool: Datenabzug lokal vorhanden\n";
...
foreach my $file(@FILES){
    if ($file=~m/export_mab_HBZ\d\d.O9039.F.(\d\d\d\d\d\d\d\d).\d+\.zip/){
        my $thisdate = $1;
...
foreach my $file(@FILES){
    if ($file=~m/export_mab_HBZ01.O9039.F.$lastdate.\d+\.zip/){
        system("unzip -v -p $pooldir/$pool/$file > $pooldir/$pool/tmp.TIT");
    }
    if ($file=~m/export_mab_HBZ10.O9039.F.$lastdate.\d+\.zip/){
        system("unzip -v -p $pooldir/$pool/$file > $pooldir/$pool/tmp.PER");
    }
    if ($file=~m/export_mab_HBZ11.O9039.F.$lastdate.\d+\.zip/){
        system("unzip -v -p $pooldir/$pool/$file > $pooldir/$pool/tmp.KOE");
    }
    if ($file=~m/export_mab_HBZ12.O9039.F.$lastdate.\d+\.zip/){
        system("unzip -v -p $pooldir/$pool/$file > $pooldir/$pool/tmp.SWD");
    }
    if ($file=~m/export_mab_HBZ60.O9039.F.$lastdate.\d+\.zip/){
        system("unzip -v -p $pooldir/$pool/$file > $pooldir/$pool/tmp.MEX");
    }
}

system("cd $pooldir/$pool; $alephmab2metaexe --titlefile='tmp.TIT' --personfile='tmp.PER' --corporatebodyfile='tmp.KOE' --subjectfile='tmp.SWD' --classificationfile='tmp.SYS' --holdingfile='tmp.MEX' --configfile='/opt/openbib/conf/$pool.yml'");
