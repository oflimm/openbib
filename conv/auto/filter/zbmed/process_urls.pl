...
while (<>){
	...
	foreach my $pmult (sort keys %$portfolio_ref){
	    if (defined $portfolio_ref->{$pmult}{'2'}){ # Dynamic URL (OpenURL Resolver) available
	    ...
	    # elsif (defined $portfolio_ref->{$pmult}{'d'}){ # Access URL
	    # 	my $url = $portfolio_ref->{$pmult}{'d'};
	    # 	$url = decode_entities($url);
	    ...
	    # }
	    elsif (defined $portfolio_ref->{$pmult}{'e'}){ # Static URL available
