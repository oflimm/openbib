...
if ($init){
    ...
    $enrichment->{schema}->resultset('EnrichedContentByIsbn')->search_rs({ field => '4101', origin => $origin })->delete;
}
        ...
        {        
            ...
            foreach my $fieldno ('084'){
                foreach my $field ($record->field($fieldno)){
                    my $content_a = ($encoding eq "MARC-8")?marc8_to_utf8($field->as_string('a')):decode_utf8($field->as_string('a'));
                    my $content_2 = ($encoding eq "MARC-8")?marc8_to_utf8($field->as_string('2')):decode_utf8($field->as_string('2'));
...
sub print_help {
    print << "ENDHELP";
bvb_rvks2enrich.pl - Anreicherung mit RVK-Informationen aus den offenen Daten des BVB
