...
#  autoconv-oai.pl
#
#  Automatische Konvertierung von OAI-Daten
#
...
$harvestoaiexe="$config{'conv_dir'}/harvestOAI.pl";
...
# Wenn kein oaiurl ueber die Kommandozeile als Argument eingegeben wurde,
# dann wird entsprechend der dboptions der oaiurl gesetzt

if (!$oaiurl){
  $oaiurl="$protocol://$host/$remotepath/$filename";
}

print "### POOL $singlepool\n";

if ($getviaoai){
  print "### $singlepool: Datenabzug via OAI von $oaiurl\n";
  system("cd $pooldir/$singlepool ; rm *.exp.gz");
  system("$harvestoaiexe --oaiurl=\"$oaiurl\" | /bin/gzip -c > $pooldir/$singlepool/pool.dat.gz");
}
...
sub print_help {
    print "autoconv-oai.pl - Automatische Konvertierung von OAI-Daten\n\n";
    print "Optionen: \n";
    print "  -help                 : Diese Informationsseite\n\n";
    print "  -get-via-oai          : Hole Pool automatisch mit OAI\n";
    print "  -fastload             : Beschleunigtes Einladen via myisamchk\n";
    print "  --single-pool=...     : Angegebenen Datenpool verwenden\n";
    print "  --oaiurl=...          : Basis-URL fuer wget\n";
    exit;
}
