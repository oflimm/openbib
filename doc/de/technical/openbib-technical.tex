...
\begin{verbatim}
msgmerge -U de/LC\_MESSAGES/openbib.po message.po
\end{verbatim}
...
Neben Xapian ließe sich alternativ eine andere
...
Diese Einfache Suche mit Such\-maschinen-Technologie zeichnet sich durch
folgendes aus:
...
Wesentlich sind hierbei die Tabellen \texttt{search} sowie
\texttt{titlistitem} für den via \texttt{Storable} serialisierten
Kurztreffereintrag, der als Data ebenfalls in den Index wandert.

Konkret wird im Xapian-Index entsprechend der Terme in
\texttt{search} gesucht und der serialisierte Kurztreffer aus
\texttt{titlistitem} zurückgeliefert.
...
\item[Definition der Standardverknüpfung] Die Standardverknüpfung
  zwischen den eingegebenen Suchbegriffen kann über die Methode
...
(Enquiry) an die Datenbank gestellt werden. Dazu werden zunächst mit der
...
(Match-Set). In diesem sind die Suchergebnisse zusammengefasst.
