...
sub new {
    ...
    # Set defaults
    my $api_password =
      exists $arg_ref->{api_password} ? $arg_ref->{api_password} : undef;

    my $api_user = exists $arg_ref->{api_user} ? $arg_ref->{api_user} : undef;

    my $api_profile =
      exists $arg_ref->{api_profile} ? $arg_ref->{api_profile} : undef;

    my $database = exists $arg_ref->{database} ? $arg_ref->{database} : '';

    my $sessionID =
      exists $arg_ref->{sessionID} ? $arg_ref->{sessionID} : undef;

    my $config =
      exists $arg_ref->{config} ? $arg_ref->{config} : OpenBib::Config->new;

    my $searchquery =
      exists $arg_ref->{searchquery}
      ? $arg_ref->{searchquery}
      : OpenBib::SearchQuery->new;

    my $queryoptions =
      exists $arg_ref->{queryoptions}
      ? $arg_ref->{queryoptions}
      : OpenBib::QueryOptions->new;
    ...
    $self->{api_user} =
        ( defined $api_user )                    ? $api_user
      : ( defined $config->{eds}{default_user} ) ? $config->{eds}{default_user}
      :                                            undef;
    $self->{api_password} =
        ( defined $api_password ) ? $api_password
      : ( defined $config->{eds}{default_password} )
      ? $config->{eds}{default_password}
      : undef;
    $self->{api_profile} =
        ( defined $api_profile ) ? $api_profile
      : ( defined $config->{eds}{default_profile} )
      ? $config->{eds}{default_profile}
      : undef;
...
sub send_retrieve_request {
    ...
    # Set defaults
    my $id = exists $arg_ref->{id} ? $arg_ref->{id} : '';
    ...
    if ( $logger->is_debug ) {
        $logger->debug( "Setting default header with x-authenticationToken: "
              . $self->{authtoken}
              . " and x-sessionToken: "
              . $self->{sessiontoken} );
    }
    ...
    eval { $json_result_ref = decode_json $response->content; };
...
sub send_search_request {
    ...
    my $sort_eds =
        $eds_reverse_sort_mapping_ref->{ $sorttype . "_" . $sortorder }
      ? $eds_reverse_sort_mapping_ref->{ $sorttype . "_" . $sortorder }
      : "relevance";
    ...
    if ($autocorrect) {
        push @search_options, "autocorrect=y";
    }
    else {
        push @search_options, "autocorrect=n";
    }
    if ($autosuggest) {
...
sub get_record {
    ...
    if ( defined $json_result_ref->{ErrorNumber}
        && $json_result_ref->{ErrorNumber} )
    {
    ...
    my $record =
      new OpenBib::Record::Title( { database => $database, id => $id } );

    my $fields_ref = ();

    # Gesamtresponse in eds_source
    push @{ $fields_ref->{'eds_source'} }, { content => $json_result_ref };
    ...
    # Allgemeine Trefferinformationen
    {
        push @{ $fields_ref->{'T0501'} },
          { content => "Datenquelle: "
              . $json_result_ref->{'Record'}{'Header'}{'DbLabel'}, }
    ...
    # Volltextlinks
    {
        ...
        eval {
            $url =
              $json_result_ref->{'Record'}{'FullText'}{'Links'}[0]{'Link'}[0]
              {'Url'};
        };
        ...
        if ($url) {    # ID=bth:94617232
            ...
            eval {
                $available = $json_result_ref->{'Record'}{'FullText'}{'Text'}
                  {'Availability'};
            };
    ...
    # arXiv, DOAJ und OAIster: Publikationstyp einfuegen und CustomLink auslesen
        if ( $json_result_ref->{Header}{DbId} =~ /^(edsarx|edsdoj|edsoai)$/ ) {
            ...
            eval {
                $url =
                  $json_result_ref->{'Record'}{'FullText'}{'CustomLinks'}[0]
                  {'Url'};
            };

            if ($url) {
                $url =~ s!(.*)\#\?$!$1!
                  ; # OAIster: "#?" am Ende entfernen, z.B. ID=edsoai:edsoai.859893876 ; ID=edsoai:edsoai.690666320
                $url =~
s!(http://etheses.bham.ac.uk/[^/]+/).*ThumbnailVersion.*\.pdf!$1!
                  ; # Sonderanpassung fuer etheses.bham.ac.uk, z.B. ID=edsoai:edsoai.690666320
...
# Hinweis pkostaedt: Der Link "Citing Articles" funktioniert nicht in jedem Fall, z.B. ID=edswss:000312205100002
        if (   $json_result_ref->{Header}{DbId} =~ /^(edswsc|edswss)$/
            && $json_result_ref->{Header}{An} )
        {
            my $url =
"http://gateway.isiknowledge.com/gateway/Gateway.cgi?&GWVersion=2&SrcAuth=EBSCO&SrcApp=EDS&DestLinkType=CitingArticles&KeyUT="
              . $json_result_ref->{Header}{An}
              . "&DestApp=WOS";
    ...
    # BibEntity
    {
        foreach my $thisfield (
            keys
            %{ $json_result_ref->{Record}{RecordInfo}{BibRecord}{BibEntity} } )
        {

            if ( $thisfield eq "Titles" ) {
                foreach my $item (
                    @{
                        $json_result_ref->{Record}{RecordInfo}{BibRecord}
                          {BibEntity}{$thisfield}
                    }
                  )
                {
            ...
            if ( $thisfield eq "Languages" ) {
                  ...
                  )
                {
                    push @{ $fields_ref->{'T0015'} },
                      { content => $item->{Text} }
                      if (
                        !$self->have_field_content( 'T0015', $item->{Text} ) );
                }
            }
            ...
            if ( $thisfield eq "Identifiers" ) {
                  ...
                  )
                {
                    ...
                    push @{ $fields_ref->{'T0010'} },
                      { content => $item->{Value} }
                      if (
                        !$self->have_field_content( 'T0010', $item->{Value} ) );
                }
            }
            ...
            if ( $thisfield eq "PhysicalDescription" ) {
                ...
                eval {
                    $startpage =
                      $json_result_ref->{Record}{RecordInfo}{BibRecord}
                      {BibEntity}{$thisfield}{Pagination}{StartPage};
                };

                eval {
                    $pagecount =
                      $json_result_ref->{Record}{RecordInfo}{BibRecord}
                      {BibEntity}{$thisfield}{Pagination}{PageCount};
                };
    ...
    {    # BibRelationships
        if (   defined $json_result_ref->{Record}{RecordInfo}{BibRecord}
            && defined $json_result_ref->{Record}{RecordInfo}{BibRecord}
            {BibRelationships} )
        {
                ...
                {BibRelationships}{HasContributorRelationships} )
            {
                  ...
                  )
                {
                    ...
                    if (   defined $item->{PersonEntity}
                        && defined $item->{PersonEntity}{Name}
                        && defined $item->{PersonEntity}{Name}{NameFull} )
                    {
                        my $name = $item->{PersonEntity}{Name}{NameFull};

                        $name =~
s{([^\(]+)\, (Verfasser|Herausgeber|Mitwirkender|Sonstige).*}{$1}
                          ; # Hinweis pkostaedt: GND-Zusaetze abschneiden, z.B. ID=edswao:edswao.47967597X
                        $name =~ s{([^\(]+)\, \(DE\-.*}{$1}
                          ; # Hinweis pkostaedt: GND-ID abschneiden, z.B. ID=edswao:edswao.417671822

                        push @{ $fields_ref->{'T0100'} }, { content => $name, }
                ...
                {BibRelationships}{IsPartOfRelationships} )
            {
                  ...
                  )
                {
                    if ( defined $partof_item->{BibEntity} ) {
                        ...
                        foreach
                          my $thisfield ( keys %{ $partof_item->{BibEntity} } )
                        {

                            if ( $thisfield eq "Titles" ) {
                                foreach my $item (
                                    @{ $partof_item->{BibEntity}{$thisfield} } )
                                {
                                    push @{ $fields_ref->{'T0451'} },
                                      { content => $item->{TitleFull} }
                                      if (
                                        !$self->have_field_content(
                                            'T0451', $item->{TitleFull}
                                        )
                                      );
...

    {
        ...
        if ( defined $json_result_ref->{'Record'}{'Items'} ) {
            ...
            foreach my $item ( @{ $json_result_ref->{'Record'}{'Items'} } ) {
                ...
                $data =~ s{<br \/>}{ ; }g;
                $data =~ s{<relatesTo>[^<]+<\/relatesTo><i>[^<]+<\/i>}{}g
                  ; # z.B. <relatesTo>2</relatesTo><i> javierm@electrica.cujae.edu.cu</i>
                $data =~ s{<i>([^<]+)<\/i>}{$1}g;
                $data =~ s{<[^>]+>([^<]+)<\/[^>]+>}{$1}g
                  ; # z.B. <searchLink fieldCode="JN" term="%22Linux%22">Linux</searchLink>
                ...
                if ( $name =~
/^(Title|Author|Language|Abstract|AbstractNonEng|TitleSource|TitleSourceBook|Publisher|DatePubCY|ISBN|ISSN|TypeDocument)$/
                  )
                {

                    if ( $name eq 'Publisher' ) {
                        $data =~ s/,\s+\d{4}$//
                          ; # z.B. edsgsl:solis.00547468 (Hamburg : Diplomica Verl., 2009 -> Hamburg : Diplomica Verl.)
                    }
                    elsif ($name eq 'TitleSource'
                        && $json_result_ref->{Header}{DbId} eq 'edsoai'
                        && $data =~ /urn:/ )
                    {
                ...
                # Normieren, hier spezielle wegen Dubletten zu  BibRelationShips
                    ...
                    if ( $label eq 'Time' ) {
                        push @{ $fields_ref->{'T0501'} }, {
                            content => "Zeitangabe: "
                              . $data
                            ,    # z.B. Geburtsdaten, ID=edsoao:oao.T045764
                        };
                    }
                    else {
                        ...
                        foreach my $subject (@subjects) {
                            push @{ $fields_ref->{'T0710'} },
                              { content => $subject, }
                              if (
                                !$self->have_field_content( 'T0710', $data ) );
                        }
                    }
                }
                elsif ( $name eq 'DOI' ) {
                    ...
                    && $label eq 'Access URL'
                    && !$is_electronic_ressource )
                {
                    my $url = '';

                    if (   $data =~ /linkTerm=.*(http.*)&lt;/
                        or $data =~ /^(http[^\s]+)/ )
                    {
                        $url = $1;
                    }

                    if ( $json_result_ref->{Header}{DbId} =~
                        /^(edsfis|edswao)$/ )
                    {    # z.B. ID=edswao:edswao.035502584
...
# SSOAR, BASE, OLC, ...: Volltext-Link auslesen, z.B. ID=edsbas:edsbas.ftunivdortmund.oai.eldorado.tu.dortmund.de.2003.30139, ID=edsgoc:edsgoc.197587160X
                        if ( $url && $url !~ /gesis\.org\/sowiport/ )
                        { # Sowiport-Links funktionieren nicht mehr, z.B. ID=edsgsl:edsgsl.793796
                            push @{ $fields_ref->{'T4120'} }, {
                                subfield =>
                                  'b',    # Eingeschraenkter Zugang / yellow
                                mult    => $link_mult,
                                content => $url
                            };
                            ...
                            if ( $json_result_ref->{Header}{DbId} eq 'edsgso' )
                            {    # SSOAR
                    ...
                    $logger->debug(
                        "URL - Label:$label - Name:$name Data:$data");
                    ...
                    foreach my $url (@urls) {
                        if ( $url =~ /doi\.org/ ) {
                            ...
                            if ( $json_result_ref->{Header}{DbId} =~
                                /^(edsoao|edsomo|edsebo|edssvl)$/ )
                            { # Links aus Grove Art und Britannica Online, z.B. ID=edsoao:oao.T045764
                                    # Todo: Zugriffstatus 'yellow' hinzufuegen
                                $availability = "yellow";
                            ...
                            my $availability_map_ref =
                              { green => 'a', yellow => 'b', unknown => '' };
                            push @{ $fields_ref->{'T4120'} }, {
                                subfield =>
                                  $availability_map_ref->{$availability}
                                ,    # Dynamisch
                                mult    => $link_mult,
                                content => $url
                            };
...
sub get_search_resultlist {
    ...
    foreach my $match_ref (@matches) {
        ...
        my $id = OpenBib::Common::Util::encode_id(
            $match_ref->{database} . "::" . $match_ref->{id} );
        my $fields_ref = $match_ref->{fields};

        $recordlist->add(
            OpenBib::Record::Title->new(
                { database => $self->get_database, id => $id }
            )->set_fields_from_storable($fields_ref)
        );
    }
...
sub get_authtoken {
    ...
    if ( $config->{memc} ) {
        ...
        if ($authtoken) {
            if ( $logger->is_debug ) {
                $logger->debug(
"Got eds authtoken $authtoken for key $memc_key from memcached"
                );
            }
            ...
            if ( $logger->is_debug ) {
                $logger->debug(
                    "No eds authtoken for key $memc_key from memcached found");
            }
...
sub get_sessiontoken {
    ...
    $logger->debug(
        "Getting sessiontoken for sessionid " . $self->{sessionID} );
    ...
    if ( $config->{memc} ) {
        ...
        if ($sessiontoken) {
            if ( $logger->is_debug ) {
                $logger->debug(
"Got eds sessiontoken $sessiontoken for key $memc_key from memcached"
                );
            }
            ...
            if ( $logger->is_debug ) {
                $logger->debug(
                    "No eds sessiontoken for key $memc_key from memcached found"
                );
            }
...
sub _create_authtoken {
    ...
    my $request =
      HTTP::Request->new( 'POST' => $config->get('eds')->{auth_url} );
    $request->content_type('application/json');
    ...
    if ( $response->is_success ) {
        ...
        if ( $json_result_ref->{AuthToken} ) {
            ...
            if ( $config->{memc} ) {
                $config->{memc}->set(
                    $memc_key,
                    $json_result_ref->{AuthToken},
                    $self->{memcached_expiration}{$memc_key}
                );

                if ( $logger->is_debug ) {
                    $logger->debug( "Saved eds authtoken "
                          . $json_result_ref->{AuthToken}
                          . " to key $memc_key in memcached" );
                }
            }
...
sub _create_sessiontoken {
    ...
    my $request =
      HTTP::Request->new( 'POST' => $config->get('eds')->{session_url} );
    $request->content_type('application/json');
    ...
    if ( $response->is_success ) {
        ...
        if ( $json_result_ref->{SessionToken} ) {
            if ( $config->{memc} ) {
                $config->{memc}->set(
                    $memc_key,
                    $json_result_ref->{SessionToken},
                    $self->{memcached_expiration}{'eds:sessiontoken'}
                );

                if ( $logger->is_debug ) {
                    $logger->debug( "Saved eds sessiontoken "
                          . $json_result_ref->{SessionToken}
                          . " to key $memc_key in memcached" );
                }
            }
...
sub process_matches {
    ...
    foreach my $match ( @{ $json_result_ref->{SearchResult}{Data}{Records} } ) {
        ...
        # Gesamtresponse in eds_source
        push @{ $fields_ref->{'eds_source'} }, { content => $match };

# $logger->debug("Processing Record ".YAML::Dump($json_result_ref->{SearchResult}{Data}{Records}));

        # Volltextlinks
        if (   defined $match->{FullText}
            && defined $match->{FullText}{CustomLinks} )
        {
            my $availability = '';
            if (   defined $match->{FullText}{Text}
                && defined $match->{FullText}{Text}{Availability} )
            {
        ...
        foreach
          my $thisfield ( keys %{ $match->{RecordInfo}{BibRecord}{BibEntity} } )
        {

            if ( $thisfield eq "Titles" ) {
                foreach my $item (
                    @{ $match->{RecordInfo}{BibRecord}{BibEntity}{$thisfield} }
                  )
                {
                    push @{ $fields_ref->{'T0331'} },
                      { content => $item->{TitleFull} }
        ...
        if (   defined $match->{RecordInfo}{BibRecord}
            && defined $match->{RecordInfo}{BibRecord}{BibRelationships} )
        {
                ...
                {HasContributorRelationships} )
            {
                foreach my $item (
                    @{
                        $match->{RecordInfo}{BibRecord}{BibRelationships}
                          {HasContributorRelationships}
                    }
                  )
                {
...
sub process_facets {
    ...
    $logger->debug( "Start processing facets: "
          . YAML::Dump( $json_result_ref->{SearchResult}{AvailableFacets} ) );

    foreach
      my $eds_facet ( @{ $json_result_ref->{SearchResult}{AvailableFacets} } )
    {
        ...
        foreach my $item_ref ( @{ $eds_facet->{AvailableFacetValues} } ) {
            push @{$contents_ref}, [ $item_ref->{Value}, $item_ref->{Count}, ];
        }

        if ( $logger->is_debug ) {
            $logger->debug(
                "Facet for type $type " . YAML::Dump($contents_ref) );
        }
...
sub have_field_content {
    ...
    eval { $have_field = $self->{have_field_content}{$field}{$content}; };
...
sub parse_query {
    ...
    foreach my $field ( keys %{ $config->{searchfield} } ) {
        my $searchtermstring =
          ( defined $searchquery->get_searchfield($field)->{val} )
          ? $searchquery->get_searchfield($field)->{val}
          : '';

        my $searchtermstring_from =
          ( defined $searchquery->get_searchfield("${field}_from")->{norm} )
          ? $searchquery->get_searchfield("${field}_from")->{norm}
          : '';
        my $searchtermstring_to =
          ( defined $searchquery->get_searchfield("${field}_to")->{norm} )
          ? $searchquery->get_searchfield("${field}_to")->{norm}
          : '';

        if ( $field eq "year"
            && ( $searchtermstring_from || $searchtermstring_to ) )
        {
            if ( $searchtermstring_from && $searchtermstring_to ) {
                ...
                push @$query_ref,
                  ...
                  . $query_count
                  . "=AND%2CDT:"
                  . cleanup_eds_query( $searchtermstring_from . "-9999" );
            }
                ...
                push @$query_ref,
                  ...
                  . $query_count
                  . "=AND&2CDT:"
                  . cleanup_eds_query( "1800-" . $searchtermstring_to );
            }
        }
            ...
            if ( defined $mapping_ref->{$field} ) {
                if ( $mapping_ref->{$field} eq "TX" ) {
                    push @$query_ref,
                      ...
                      . $query_count
                      . "=AND%2C"
                      . cleanup_eds_query($searchtermstring);
                }
                else {
...
#push @$query_ref, "query-".$query_count."=AND%2C".cleanup_eds_query($mapping_ref->{$field}.":".$searchtermstring);
    ...
    if ( $logger->is_debug ) {
        $logger->debug(
            "All filters: " . YAML::Dump( $searchquery->get_filter ) );
    }

    my $eds_reverse_facet_mapping_ref =
      $config->get('eds_reverse_facet_mapping');

    if ( @{ $searchquery->get_filter } ) {
        $filter_ref = [];
        foreach my $thisfilter_ref ( @{ $searchquery->get_filter } ) {
            my $field =
              $eds_reverse_facet_mapping_ref->{ $thisfilter_ref->{field} };
            my $term = $thisfilter_ref->{term};

            #            $term=~s/_/ /g;

            $logger->debug( "Facet: $field / Term: $term (Filter-Field: "
                  . $thisfilter_ref->{field}
                  . ")" );

            if ( $field && $term ) {
                push @$filter_ref, "facetfilter="
                  . cleanup_eds_filter( $filter_count . ",$field:$term" );
                $filter_count++;
            }
        }
