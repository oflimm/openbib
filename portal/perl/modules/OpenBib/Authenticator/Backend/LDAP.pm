...
sub authenticate {
	...
	$logger->debug("Checking $userdn in LDAP-Tree.");
	my $mesg = $ldaps->bind(
	    $userdn, 
	    password => "$password"
	    );
	...
	if ($mesg && $mesg->code() == 0){
	    ...
	    my $ldapfilter = qq(sAMAccountName=$username);
	    
	    my $result = $ldaps->search(
		base   => $authenticator_config->{basedn},
		filter => qq($ldapfilter),
		);
		...
		if ($logger->is_debug){
		    ...
		    if ($authenticator_config->{restrict_groupdn}){
			$success = 0;
			foreach my $thismember ($entry->get_value('memberOf')){
			    if ($authenticator_config->{restrict_groupdn}{$thismember}){
				$success = 1;
			    }
			}
		    }
