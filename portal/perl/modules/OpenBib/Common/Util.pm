...
Liefert den Namen des CSS Stylesheets entsprechend des aufrufenden HTTP_USER_AGENT zur�ck. Das ist im Fall der aktuellen MSIE-Versionen 5-9 das Stylesheet openbib-ie.css, im Fall von Mozilla 5.0 das Stylesheet openbib.css. Bei �lteren Browser-Version wird im Falle von MSIE sonst openbib-simple-ie.css bzw. bei allen anderen Browsern openbib-simple.css verwendet.

=item grundform({ content => $content, $category => $category, searchreq => $searchreq, tagging => $tagging})

Allgemeine Normierung des Inhaltes $content oder in Abh�ngigkeit von der Kategorie $category, bei einer Suchanfrage ($searchreq=1) bzw. beim Tagging ($tagging=1). Neben einer Filterung nach erlaubten Zeichen erfolgt insbesondere die R�ckf�hrung von Zeichen auf ihre Grundbuchstaben, also ae f�r � oder e f�r �.

=item get_loadbalanced_servername

Liefert den Namen des Servers aus der Menge aktiver Produktionsserver zur�ck, der am wenigsten belastet ist (bzgl. Load) und dessen Session-Datenbank korrekt funktioniert.

=item normset2bibtex($normset_ref,$utf8) 

Wandelt den bibliographischen Datensatz $normset_ref in das BibTeX-Format um. �ber $utf8 kann spezifiziert werden, ob in diesem Eintrag UTF8-Kodierung verwendet werden soll oder plain (La)TeX.
...
Generiere die Basiszeichenkette aus den bibliographischen Daten f�r die Bildung des BibKeys. Dies ist eine Hilfsfunktion f�r gen_bibkey
...
Erzeuge eine ISBN13 aus einer ISBN und liefere diese normiert (keine Leerzeiche oder Bindestricke, Kleinschreibung) zur�ck. 

=item get_cascaded_templatepath({ database => $database, view => $view, profile => $profile, templatename => $templatename })

Liefert in Abh�ngigkeit der Datenbank $database, des View $view und des Katalogprofils $profile den effektiven Pfad zum jeweiligen Template $templatename zur�ck.

=item gen_cloud_class({ items => $items_ref, min => $mincount, max => $maxcount, type => $type})

Reichere eine Liste quantifizierter Begriffe $items_ref entsprechend schon bestimmten minimalen und maximalen Vorkommens $mincount bzw. $maxcount f�r den type 'linear/log' mit Klasseninformatinen f�r die Bildung einer Wortwolke an. 

=back

=head2 Ausgabe �ber Apache-Handler
...
Ausgabe des Templates $templatename mit den Daten $ttdata �ber den Apache-Handler $r

=item print_warning($warning,$r,$msg)

Ausgabe des Warnhinweises $warning �ber den Apache-Handler $r unter Verwendung des Message-Katalogs $msg
