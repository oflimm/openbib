...
sub handler {
    ...
    if ($type eq "tit" && $id){
        ...
        foreach my $swtid (@swtids){
            $request = $dbh->prepare("select distinct c2.targetid as id from conn as c1 left join conn as c2 on c1.sourceid=c2.sourceid where c1.sourcetype=1 and c2.sourcetype=1 and c1.targettype=4 and c2.targettype=4 and c1.targetid=? and c2.targetid != ?");
