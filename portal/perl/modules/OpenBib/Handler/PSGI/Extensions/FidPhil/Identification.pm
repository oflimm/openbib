...
package OpenBib::Handler::PSGI::Extensions::FidPhil::Identification;
...
sub show_form {
  ...
  #'none would not evaluate to false here !?
  #if ($user->{ID} && !$validtarget){
   #  $userData = new OpenBib::User({ID => $user->{ID} });
   #  $userinfo = $userData->get_info;
   #  $user_roles = $userData->get_roles_of_user;
   #  $ttdata->{userinfo} = $userinfo;
   #  $ttdata->{user_roles} = $user_roles;
   #  $ttdata->{stringdata} = "";
   #  my $hasFiDRole = 0;
   ...
   #      $hasFiDRole = 0;
   #    }
   #    elsif ($role eq 'society_user_pending'){
...
# }
 ...
 return $self->print_page("users_identification",$ttdata);
}
