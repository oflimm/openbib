...
sub authenticate {
    ...
    my $validtarget =
      ( $query->param('validtarget') ) ? $query->param('validtarget') : 'none';
...
"Ihre Kennung ist nicht zur Nutzung dieses Portals zugelassen. Cannot access view"
        ...
        my $redirecturl =
"$path_prefix/$config->{users_loc}/id/[% user.ID %]/preferences.html?l=$lang";
    ...
    if ( $userid > 0 ) {
        ...
        if ( $self->param('representation') eq "html" ) {
            ...
            if ( !$authorized_user->can_access_view($view) ) {
                my $reason = $msg->maketext(
"Ihre Kennung ist nicht zur Nutzung dieses Portals zugelassen."
                );
                if ( $self->param('representation') eq "html" ) {
                    return $self->print_warning($reason);
                }
            ...
            my $recordlist_existing_collection =
              $session->get_items_in_collection();
...
"$path_prefix/$config->{users_loc}/id/$userid/preferences.html?l=$lang";
    ...
    if ($redirect_to) {
        $redirecturl = $redirect_to."?userid=$userid";
    }
