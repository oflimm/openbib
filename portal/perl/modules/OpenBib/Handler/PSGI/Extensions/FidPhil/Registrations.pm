...
package OpenBib::Handler::PSGI::Extensions::FidPhil::Registrations;
...
sub show {
    ...
    my $targetid
        = ( $query->param('targetid') ) ? $query->param('targetid') : 'none';
    my $username
        = ( $query->param('username') ) ? $query->param('username') : '';
    my $password1
        = ( $query->param('password1') ) ? $query->param('password1') : '';
    my $password2
        = ( $query->param('password2') ) ? $query->param('password2') : '';
    ...
    return $self->print_page( $config->{tt_users_registrations_tname},
        $ttdata );
}
...
sub mail_confirmation {
    ...
    if ( $username eq "" || $password1 eq "" || $password2 eq "" ) {
        $ttdata->{errors}->{general}
            = $msg->maketext(
            "Es wurde entweder kein Benutzername oder keine zwei Passworte eingegeben"
            );
        return $self->print_page( $config->{tt_users_registrations_tname},
            $ttdata );
    }
    ...
    if ( $password1 ne $password2 ) {
        $ttdata->{errors}->{password} = $msg->maketext(
            "Die beiden eingegebenen Passworte stimmen nicht überein.");
        return $self->print_page( $config->{tt_users_registrations_tname},
            $ttdata );
    }
    ...
    unless ( Email::Valid->address($username) ) {
        $ttdata->{errors}->{email} = $msg->maketext(
            "Sie haben keine gültige Mailadresse eingegeben");
        return $self->print_page( $config->{tt_users_registrations_tname},
            $ttdata );
    }
    ...
    {
        $ttdata->{errors}->{email} = $msg->maketext(
            "Ein Benutzer mit dem Namen $username existiert bereits.");
        return $self->print_page( $config->{tt_users_registrations_tname},
            $ttdata );
    }
    ...
    if ( $config->{recaptcha_private_key} ) {
        ...
        # Recaptcha pruefen
        my $recaptcha_result
            = $recaptcha->check_answer_v2( $config->{recaptcha_private_key},
            $recaptcha_response, $client_ip );

        unless ( $recaptcha_result->{is_valid} ) {
            return $self->print_warning(
                $msg->maketext(
                    "Sie haben ein falsches Captcha eingegeben! Gehen Sie bitte [_1]zurück[_2] und versuchen Sie es erneut.",
                    "<a href=\"$path_prefix/$config->{users_loc}/$config->{registrations_loc}.html\">",
                    "</a>"
                )
            );
        }
    ...
    my $registrationid
        = $user->add_confirmation_request(
        { username => $username, password => $password1, viewname => $view }
        );
    ...
    # Mixed Bag wird übergangsweise hier eingefügt
    my $registration = $user->get_schema->resultset('Registration')
        ->search_rs( { id => $registrationid } );
    $registration->update( { mixed_bag => $input_data_ref->{mixed_bag} } );
    ...
    $maintemplate->process(
        $config->{tt_users_registrations_mail_message_tname}, $mainttdata )
        || do {
   ...
   #$mailmsg->send('sendmail', "/usr/bin/mail -s -f$config->{contact_email}");
    ...
    return $self->print_page(
        $config->{tt_users_registrations_confirmation_tname}, $ttdata );
}
...
sub register {
    ...
    my $view = $self->param('view') || '';
    my $registrationid
        = $self->strip_suffix( $self->param('registrationid') ) || '';
    ...
    my $confirmation_info_ref = $user->get_confirmation_request(
        { registrationid => $registrationid } );
    ...
    if ( $username && $viewid && $hashed_password ) {
            ...
            )
        {
            return $self->print_warning(
                $msg->maketext(
                    "Ein Benutzer mit dem Namen [_1] existiert bereits. Haben Sie vielleicht Ihr Passwort vergessen? Dann gehen Sie bitte [_2]zurück[_3] und lassen es sich zumailen.",
                    "$username",
                    "<a href=\"http://$r->get_server_name$path_prefix/$config->{users_loc}/$config->{registrations_loc}.html\">",
                    "</a>"
                )
            );
        }
        ...
        return $self->print_warning(
            $msg->maketext(
                "Diese Registrierungs-ID existiert nicht für dieses Portal.")
        );
    }

    # TT-Data erzeugen
    my $ttdata = { username => $username, };

    return $self->print_page( $config->{tt_users_registrations_success_tname},
        $ttdata );
}
