...
sub mail_confirmation {
    ...
    if ($username eq "" || $password1 eq "" || $password2 eq "") {
        $ttdata->{errors}->{general} = $msg->maketext("Es wurde entweder kein Benutzername oder keine zwei Passworte eingegeben");
        return $self->print_page($config->{tt_users_registrations_tname},$ttdata);
    }

    if ($password1 ne $password2) {
        $ttdata->{errors}->{password} = $msg->maketext("Die beiden eingegebenen Passworte stimmen nicht überein.");
        return $self->print_page($config->{tt_users_registrations_tname},$ttdata);
    }
    
    # Ueberpruefen, ob es eine gueltige Mailadresse angegeben wurde.
    unless (Email::Valid->address($username)){
        $ttdata->{errors}->{email} = $msg->maketext("Sie haben keine gültige Mailadresse eingegeben");
        return $self->print_page($config->{tt_users_registrations_tname},$ttdata);
   }
    ...
    if ($user->user_exists_in_view({ username => $username, viewname => $view, authenticatorid => $authenticator_self_ref->{id}})) {
        $ttdata->{errors}->{email} = $msg->maketext("Ein Benutzer mit dem Namen $username existiert bereits.");
        return $self->print_page($config->{tt_users_registrations_tname},$ttdata);
    }
