#####################################################################
#
#  OpenBib::Importer::JSON::Classification.pm
#
#  Notationen
#
...
package OpenBib::Importer::JSON::Classification;
...
sub process {
    ...
    my $inverted_ref  = $self->{conv_config}{inverted_classification};
    my $blacklist_ref = $self->{conv_config}{blacklist_classification};
    ...
    if ($mainentry) {
        $self->{storage}{listitemdata_classification}{$id}=$mainentry;
    }
        ...
        foreach my $item_ref (@{$fields_ref->{$field}}) {
            if (exists $inverted_ref->{$field}->{index}) {
                foreach my $searchfield (keys %{$inverted_ref->{$field}->{index}}) {
                    ...
                    my $hash_ref = {};
                    if (exists $self->{storage}{indexed_classification}{$id}) {
                        $hash_ref = $self->{storage}{indexed_classification}{$id};
                    }
                    push @{$hash_ref->{$searchfield}{$weight}}, ["N$field",$item_ref->{content}];
                    
                    $self->{storage}{indexed_classification}{$id} = $hash_ref;
                }
            }
