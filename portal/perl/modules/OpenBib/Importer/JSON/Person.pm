...
sub process {
    ...
    my $inverted_ref  = $self->{conv_config}{inverted_person};
    my $blacklist_ref = $self->{conv_config}{blacklist_person};
    ...
    if ($mainentry) {
        $self->{storage}{listitemdata_person}{$id}=$mainentry;
    }
        ...
        foreach my $item_ref (@{$fields_ref->{$field}}) {
            if (exists $inverted_ref->{$field}->{index}) {
                foreach my $searchfield (keys %{$inverted_ref->{$field}->{index}}) {
                    ...
                    my $hash_ref = {};
                    if (exists $self->{storage}{indexed_person}{$id}) {
                        $hash_ref = $self->{storage}{indexed_person}{$id};
                    }
                    push @{$hash_ref->{$searchfield}{$weight}}, ["P$field",$item_ref->{content}];
                    
                    $self->{storage}{indexed_person}{$id} = $hash_ref;
                }
            }
