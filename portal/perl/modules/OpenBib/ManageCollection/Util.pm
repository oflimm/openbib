...
sub titset_to_text {
  ...
  foreach my $item (@$normset){
    @{$item}{desc}=sgml2umlaut(@{$item}{desc});
    @{$item}{contents}=sgml2umlaut(@{$item}{contents});
    while (length(@{$item}{desc}) < 24){
      @{$item}{desc}.=" ";
    }
  }
...
sub titset_to_endnote {
  ...
  my @tempset=();
  foreach my $item (@$normset){
    if (exists $endnote{@{$item}{desc}}){
      @{$item}{desc}=$endnote{@{$item}{desc}};
      @{$item}{contents}=sgml2umlaut(@{$item}{contents});
      push @tempset, $item;
    }
  }
