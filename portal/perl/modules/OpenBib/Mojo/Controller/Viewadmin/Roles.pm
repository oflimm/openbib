#####################################################################
#
#  OpenBib::Mojo::Controller::Viewadmin::Roles
#
...
package OpenBib::Mojo::Controller::Viewadmin::Roles;
...
sub show_collection {
    ...
    my $roleinfos_ref = $config->get_roleinfo_of_viewadmin($view);
    
    my $ttdata = {
        roleinfos => $roleinfos_ref,
    };
    
    return $self->print_page($config->{tt_viewadmin_roles_tname},$ttdata);
}
...
sub show_record_form {
    ...
    if (!$user->role_exists_in_view({ viewname => $view, rolename => $roleid})){
        return $self->print_authorization_error();
    }
    ...
    return $self->print_page($config->{tt_viewadmin_roles_record_edit_tname},$ttdata);
}
...
sub show_record {
    ...
    return $self->print_page($config->{tt_viewadmin_roles_record_tname},$ttdata);
}
...
sub create_record {
    ...
    # Set current view
    $input_data_ref->{viewadmins} = [$view];
    ...
    if ($self->stash('representation') eq "html"){
        # TODO GET?
        $self->redirect("$path_prefix/$config->{viewadmin_loc}/$config->{roles_loc}/id/$input_data_ref->{rolename}/edit.html?l=$lang");
        return ;
    }
    else {
...
sub confirm_delete_record {
    ...
    return $self->print_page($config->{tt_viewadmin_roles_record_delete_confirm_tname},$ttdata);
}
