...
sub new {
    ...
    $self->{valid_lang_code} = $valid_lang_code;
    $self->{lang_replacements} = \%lang_replacements;
    $self->{langs_to_replace} = qr/$langs_to_replace/;

    $logger->info("Normalizer created");
...
sub normalize_lang {
    ...
    return $inputlang if (defined $self->{valid_lang_code}{$inputlang});
    ...
    if ($self->{valid_lang_code}{$inputlang}){
        return $inputlang;
    }
