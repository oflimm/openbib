...
sub connectDB {
    ...
    eval {
        ...
        $self->{schema} = OpenBib::Schema::Statistics::Singleton->connect("DBI:Pg:dbname=$config->{statisticsdbname};host=$config->{statisticsdbhost};port=$config->{statisticsdbport}", $config->{statisticsdbuser}, $config->{statisticsdbpasswd},$config->{statisticsdboptions}) or $logger->error_die($DBI::errstr);
    };
