...
my $litlistitems = $user->get_schema->resultset('Litlistitem');

while (my $item = $litlistitems->next()){
    my $titleid    = $item->get_column('titleid');
    my $database   = $item->get_column('dbname');    
    my $titlecache = $item->get_column('titlecache');    

    # Testen, ob JSON-Cache umgewandelt werden kann
    eval {
	my $json_ref = JSON::XS::decode_json $titlecache;
    };
    ...
    if ($@){
		...
		if ($new_titlecache){
		    print "$new_titlecache\n";
		    $item->update({ titlecache => $new_titlecache, comment => 'fixed from title' });
		    print STDERR "Cache aktualisiert fuer DB $database - ID $titleid mit aktuellen Titeldaten\n";	    
		}
	    }
	    # sonst versuche reencoding
	    else {
		my $reencodedcache = encode('UTF-8',$titlecache);
		
		eval  {
		    my $json_ref = decode_json $reencodedcache;		    
		};

		if ($@){
		    print STDERR "Fehler fuer DB $database - ID $titleid\n";
		}		
		else {
		    $item->update({ titlecache => $reencodedcache, comment => 'fixed encoding' });
		    print STDERR "Cache aktualisiert fuer DB $database - ID $titleid mit korrigiertem Encoding\n";	    
		}
	    }
	}
    }
