...
my $msg = OpenBib::L10N->get_handle('de');
...
my $request=$dbh->prepare("select distinct id from tit") or $logger->error($DBI::errstr);
...
foreach $titidn (keys %titidns){
    my $record = new OpenBib::Record::Title({database => $database, id => $titidn})->load_full_record();
...
my $outputbasename="katalog-$database";
...
$template->process("katalog_$mode", $ttdata) || do { 
    print $template->error();
};

sub print_help {
    print "gen-katalogpdf.pl - Erzeugen von PDFs aus einem Katalog\n\n";
    print "Optionen: \n";
    print "  -help                   : Diese Informationsseite\n";
    print "  --database=olgkoeln     : Datenbankname des Katalogs\n";
