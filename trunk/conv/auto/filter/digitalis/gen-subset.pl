...
my $dbh=DBI->connect("DBI:$config->{dbimodule}:dbname=inst137;host=$config->{dbhost};port=$config->{dbport}", $config->{dbuser}, $config->{dbpasswd}) or $logger->error_die($DBI::errstr);

# IDN's der Exemplardaten und daran haengender Titel bestimmen

print "### $pool: Bestimme Titel-ID's anhand des Links auf 'www.digitalis.uni-koeln.de' in 662\n";

my $request=$dbh->prepare("select distinct tit.id as titid from tit where category=662 and content like 'http://www.digitalis.uni-koeln.de%'") or $logger->error($DBI::errstr);
...
open(AUT,"gzip -dc $pooldir/inst137/unload.PER.gz|");
...
open(KOR,"gzip -dc $pooldir/inst137/unload.KOE.gz|");
...
open(NOTA,"gzip -dc $pooldir/inst137/unload.SYS.gz|");
...
# Schlagworte
open(SWT,"gzip -dc $pooldir/inst137/unload.SWD.gz|");
...
open(TIT,"gzip -dc $pooldir/inst137/unload.TIT.gz|");
...
open(MEX,"gzip -dc $pooldir/inst137/unload.MEX.gz|");
