...
system("cd $pooldir/$pool ; cat noi-header.xml > $pooldir/$pool/pool.xml ; cat `ls -1 noi-records-*.xml|sort -r |xargs` >> $pooldir/$pool/pool.xml ; cat noi-footer.xml >> $pooldir/$pool/pool.xml");
